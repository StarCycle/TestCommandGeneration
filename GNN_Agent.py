...
class GNN_Agent(torch.nn.Module):
    def __init__(self, env, out_channels, gnn_layers, device):
        ...
        self.graphmlp = nn.Sequential(
            layer_init(nn.Linear(self.nodes.shape[0]+self.historyVecLen, 128)),
            nn.ReLU(),
        )
        self.value_net = layer_init(nn.Linear(128, 1), 0.01)
        self.advantage_net = layer_init(nn.Linear(128, len(env.actions)), 0.01)
