...
class Agent(nn.Module):
    def __init__(self, env, num_nn, critic_std, actor_std):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(len(env.recordCov+env.actions), num_nn)), # My environment
...
if __name__ == "__main__":
    ...
    gamma = [0.9]
    ent_coef = [0.005, 0.01]  # Weight of the entropy loss in the total loss
    vf_coef = [0.5]			# Weight of the value loss in the total loss
    num_nn = [1024]
    critic_std = [1]
    actor_std = [0.01]
    num_epoch_steps = 256	# How many steps you interact with the env before a reset
